...
window.toggleIconSelection = function() {
    const container = document.getElementById('icon-grid-container');
    const button = document.querySelector('.icon-toggle-btn');
...
window.selectIcon = function(icon) {
    ...
    document.querySelector(`[onclick="selectIcon('${icon}')"]`).classList.add('selected');
};

// Calendar functions with improved grid layout
window.setDateMode = function(mode) {
    dateMode = mode;
    document.querySelectorAll('.date-mode-btn').forEach(btn => btn.classList.remove('active'));
...
function renderCalendar() {
    ...
    // Next month days to fill remaining cells
    const totalCells = daysContainer.children.length;
    const remainingCells = (42 - totalCells) % 7; // Fill to complete weeks
    if (remainingCells > 0) {
        const nextMonth = new Date(currentCalendarDate.getFullYear(), currentCalendarDate.getMonth() + 1, 1);
        for (let day = 1; day <= remainingCells; day++) {
            const date = new Date(nextMonth.getFullYear(), nextMonth.getMonth(), day);
...
window.editCategoryInline = function(categoryId, type) {
    ...
    displayCategories(); // Refresh to show edit mode
};
...
// Enhanced account management with absolute balance
...
window.saveEditAccount = function() {
    ...
    // Calculate the difference for transaction recording
    const balanceDifference = newBalance - oldBalance;
    account.balance = newBalance;

    // Record balance change as transaction if there's a difference
    ...
    // Record account details change if name or icon changed
